...
# Ubuntu/Debian
if [ $PKG = deb ]
  then apt-get install git libxml2-dev libxslt-dev libcurl4-openssl-dev libpq-dev libsqlite3-dev build-essential postgresql libreadline-dev

# CentOS/Fedora
elif [ $PKG = rpm ]
  then yum install git libxml2-devel libxslt-devel curl-devel postgresql-devel sqlite-devel  make automake gcc gcc-c++ postgresql-server readline-devel openssl-devel

  service postgresql initdb && service postgresql start

# Arch Linux
elif [ $PKG = pkg ]
  then pacman -S git postgresql base-devel libxml2 libxslt curl sqlite readline postgresql-libs
