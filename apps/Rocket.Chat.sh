...
if [ $ARCH = amd64 ] || [ $ARCH = 86 ]
  then . $DIR/sysutils/supervisor.sh Rocket.Chat "sh -c \"ROOT_URL=http://$IP:3000/ MONGO_URL=mongodb://localhost:27017/rocketchat PORT=3000 $HOME/meteor/dev_bundle/bin/node $HOME/Rocket.Chat/bundle/main.js\"" /root/Rocket.Chat
elif [ $ARCH = arm ] || [ $ARCH = armv6 ]
  then . $DIR/sysutils/supervisor.sh Rocket.Chat "sh -c \"ROOT_URL=http://$IP:$port/ MONGO_URL=mongodb://localhost:27017/rocketchat PORT=$port $HOME/meteor/dev_bundle/bin/node $HOME/Rocket.Chat/bundle/main.js\"" /root/Rocket.Chat/bundle
fi
